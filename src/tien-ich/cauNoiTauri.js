...
export async function taiCauHinhMau() {
  try {
    ...
    console.error("VNMark — không đọc được cấu hình mẫu đã lưu, sẽ dùng mẫu mặc định:", loi);
    return null;
  }
}
...
export async function luuCauHinhMau(danhSachMau) {
  try {
    ...
    console.error("VNMark — không lưu được cấu hình mẫu (sẽ chỉ tồn tại tạm trong phiên làm việc này):", loi);
  }
}
